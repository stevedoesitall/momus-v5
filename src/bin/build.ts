...
// Convert SCSS files
child_process.execSync("sass public/styles/scss/global.scss public/styles/css/global.css");

// Remove unnecessary CSS
fs.mkdirSync("./build/public/styles/");
fs.unlinkSync("./public/styles/css/final.css");
fs.mkdirSync("./public/styles/purged");

child_process.execSync("purgecss --css public/styles/css/*.css --content views/layout.ejs views/**/*.ejs --output public/styles/purged");

// Concatenate the purged CSS files
fs.writeFileSync("./public/styles/css/final.css" , "");
...
publicStyleFiles.forEach(file => {
	if (!file.includes("final.") && !file.includes(".map")) {
		...
		fs.appendFileSync("./public/styles/css/final.css", contents + "\n");
	}
...
child_process.execSync("esbuild ./build/public/scripts/index.js --bundle --outfile=./build/public/scripts/bundle.js --minify --sourcemap --target=chrome58,firefox57,safari11,edge16 --format=esm");
child_process.execSync("esbuild ./public/styles/css/final.css --bundle --outfile=./build/public/styles/bundle.css --minify --sourcemap");
...
const buildScriptFiles = fs.readdirSync("./build/public/scripts/");

buildScriptFiles.forEach(file => {
	const filePath = `./build/public/scripts/${file}`;
	if (!file.includes("bundle.")) {
		fs.unlinkSync(filePath);
	}
});
